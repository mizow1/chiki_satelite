...
function form_submit(nc_flag,target_count,sp_flag){
	if(SubmitCount==1){
		return ;
	}
	SubmitCount=1;
	...
	// form_data.find("input[name='sp_flag']").attr("value",sp_flag);
...
cookie_data['birthday1'] ="";
cookie_data['birthday2'] ="";
...
//	var pref_obj = document.getElementById('pref');
	...
	if(cookie_data['sex1'] == 2){
		document.getElementById('sex12').selected = true;
	}else{
		document.getElementById('sex11').selected = true;
	}
	...
	if(cookie_data['birthday1'] != ''){
		document.getElementById('birthday1').value=cookie_data['birthday1'];
	}
	...
	if(target_flag==2){
		...
		if(cookie_data['sex2'] == 2){
			document.getElementById('sex22').selected = true;
		}else{
			document.getElementById('sex21').selected = true;
		}

		if(cookie_data['birthday2'] != ''){
			document.getElementById('birthday2').value=cookie_data['birthday2'];
		}
...
// クッキーの発行（書き込み）
	...
	document.cookie = 'birthday1='   + document.getElementById('birthday1').value+ " ; expires=" + expires;
...
console.log('426a',document.getElementById('sex11').selected);
	if(document.getElementById('sex11').selected){
	...
	if(target_flag==2){
		...
		if(document.getElementById('sex21').selected){
		...
		document.cookie = 'birthday2='   + document.getElementById('birthday2').value+ " ; expires=" + expires;
...
// クッキーの発行（書き込み）
	...
	document.cookie = 'birthday1' + "= ----/--/--; expires=" + expires;
	...
	document.cookie = 'birthday2' + "= ----/--/--; expires=" + expires;


	document.cookie = 'sex1' + "= 2; expires=" + expires;
	document.cookie = 'sex2' + "= 1; expires=" + expires;
	document.cookie = 'default_check' + "=0 ; expires=" + expires;
console.log('482a',document.cookie);
}
