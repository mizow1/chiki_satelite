<?php
function smarty_function_sp_widget($params, &$template){
	...
	if(!$ow_system_auto_widget_file_flag || !$ow_system_auto_template_id_flag){
		require_once(MODEL.'front/template/HtmlTemplateModelClass.php');
		$SPFrontHtmlTemplate = new SPFrontHtmlTemplateModel();
		$template_data = $SPFrontHtmlTemplate->getOpenHtmlTemplateData($template_name, TEMPLATE_TYPE_PARTS);
	...
	$class_file = WIDGET.'pc/'.$class_name.'.php';
	if (file_exists($class_file)) {
		require_once(SMARTY_PLUGINS_DIR.'shared.get_sp_widget_params.php');
		$params = smarty_get_sp_widget_params($widget_name, $params);
