<?php
function smarty_function_qreki5($params, &$template){
	$r = new Qreki();
	$year = empty($params["year"])?date('Y'):$params["year"];
	$month = empty($params["month"])?date('n'):$params["month"];
	$day = empty($params["day"])?date('j'):$params["day"];
	$rokuyou = $r->get_rokuyou($year, $month, $day);
	echo $rokuyou;
}
...
// http://www.2chan.net/script/を元にしています
// class(PHP5)化
// 円周率をpi()に変更 精度以上の数字を定義しても意味ないので
// int()関数を(int)型キャストに変更
// ユリウス通日の処理にカレンダー関数を使用（JD2YMDTは削除）
// 六曜の戻値を文字列に変更
// バグ修正
//
// arranged by 俺
// 2005/07/15 旧暦6月10日（先負）
// 2006/03/29 calendar関数が使えない場合のためにYMDT2JD関数を修正して復活

if (!extension_loaded('calendar')) {
    #=========================================================================
    # 年月日（世界時）からユリウス日（JD）を計算する
    #
...
#
# 引数：新暦年月日
# 戻値：大安 赤口 先勝 友引 先負 仏滅
#
#=========================================================================
    public function get_rokuyou($year,$mon,$day)
    {
        $rokuyou = array('大安','赤口','先勝','友引','先負','仏滅');
...
#=========================================================================
    ...
    {
        $tm0 = GregorianToJD($mon,$day,$year)-1;
#-----------------------------------------------------------------------
...
#
#=========================================================================
    protected function calc_chu($tm)
    {
#-----------------------------------------------------------------------
#時刻引数を分解する
#-----------------------------------------------------------------------
...
#=========================================================================
    protected function before_nibun($tm)
    {
#-----------------------------------------------------------------------
#時刻引数を分解する
#-----------------------------------------------------------------------
...
#-----------------------------------------------------------------------
# 戻り値の作成
# nibun[0]:時刻引数を合成するのと、DT ==> JST 変換を行い、戻り値とする
# （補正時刻=0.0sec と仮定して計算）
# nibun[1]:黄経
#-----------------------------------------------------------------------
...
#
#=========================================================================
    protected function calc_saku($tm)
    {
#-----------------------------------------------------------------------
# ループカウンタのセット
#-----------------------------------------------------------------------
...
#=========================================================================
    protected function NORMALIZATION_ANGLE($angle)
    {
        if( $angle < 0.0 ){
            $angle1 = -$angle;
...
# 太陽の黄経 λsun を計算する
#=========================================================================
    protected function LONGITUDE_SUN( $t )
    {
#-----------------------------------------------------------------------
...
# 月の黄経 λmoon を計算する
#=========================================================================
    protected function LONGITUDE_MOON( $t )
    {
#-----------------------------------------------------------------------
# （角度の）度からラジアンに変換する係数の定義
#-----------------------------------------------------------------------
...
#
#=========================================================================
    public function check_24sekki($year,$mon,$day)
    {
#-----------------------------------------------------------------------
# ２４節気の定義
#-----------------------------------------------------------------------
        ...
        $tm = GregorianToJD($mon,$day,$year)-1;
#-----------------------------------------------------------------------
#時刻引数を分解する
#-----------------------------------------------------------------------
        $tm1 = $tm;
